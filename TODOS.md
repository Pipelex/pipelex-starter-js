...
Working tracker for this burn-down, committed alongside the work and archived via wip-triage once it lands. Source items: `wip/_top_priorities.md` P1 + P2, backed by the archived review record `../wip/history/codegen/pipelex-starter-js-codegen-followups-pr18.md` and `wip/pr-19-review-notes.md`.
...
- [x] Commit (`5c06021`, after an amend). (Full `main()` coverage remains the parked main-guard item — the moved helpers carry the test weight.)
...
- [ ] Archive this tracker (wip-triage) once the work lands on `dev`.
...
>
> **Record (2026-08-20):** One PR instead of two — PR #19 merged before Phase 1 ran, so everything landed on **`feature/Codegen-follow-ups`** → **PR #20** (targets `dev`). Phase commits: `5c42288` (Phase 1, shared-test decoupling), `4edd176` (Phase 2, shared layer + tests), `5c06021` (Phase 3, check rewrite), `322bf2f` (Phase 4, scheme guard wiring + server-only), `64876ca` (Phase 5, docs + changelog).
>
> **Decision changes en route.** `assertSecureBaseUrl` landed with Phase 2 rather than Phase 4, because it lives in the shared module; the tagged error classes declare their fields explicitly, because Node's strip-types mode rejects TypeScript parameter properties (vitest transpiles, so only the live script caught it); `findOrphanTrees` also refuses symlinks and special files at the `src/generated/` root, which the plan had specified only for the `methods/` side.
>
> **Review rounds.** Greptile ran three times and each round found a real hole in the symlink policy, each narrower than the last: the root-level `readdir` followed a symlinked `methods/` or `src/generated/` root (`0d387c8`), `writeTree` wrote artifacts before refusing a symlinked per-method tree (`e304378`), and a symlinked directory _nested_ inside an otherwise regular tree still received writes, fixed by vetting the whole pre-existing tree with `walk` before the first write (`840c43b`). Final verdict 5/5, "safe to merge". Cubic could not review at all — the workspace has reached its free monthly review limit — so it never gave a verdict on this branch. ChatGPT-Codex raised one P1, asking the shared execution-helper tests to use the generated binders instead of inline fixture narrowers; **rejected on record**, because that coupling is exactly what Phase 1 removed, and the thread carries the rationale.
>
> **Final pre-landing review** (a fresh no-context pass over `dev...HEAD` against this plan) confirmed every ticked item is delivered and found no scope creep. It produced one real defect and a set of accuracy fixes: the writer trusted every server-named `artifact.path` into `path.join`, so a response naming `../../../.husky/pre-commit` would have written outside `src/generated/` — closed by `isContainedPath` beside the existing `lock_filename` guard, refusing the method whole before the first write (`1f60cc9`). Three comments had gone stale against their own code and one changelog entry carried a hardcoded count (`cceb3ef`, `1f60cc9`); the shared layer's per-entry policy at the two scanned roots was untested (`1500a5e`).
>
> **Deferred, not dropped.** Five judgment calls are recorded with `file:line` evidence in `wip/pr-20-review-notes.md` and promoted into `wip/_top_priorities.md` under P3, so they outlive both machine-local files: the `methodsDir` parameter's hardcoded `methods/` label, the tests' Windows-hostile `symlink()` calls, the raw-stack presentation of shared-layer refusals in `codegen`/`codegen:verify` (which in the writer can leave a half-regenerated tree, so it waits on the parked atomicity call), the raw ENOENT on an absent `methods/` (which collides with the parked empty-`methods/` question), and `assertSecureBaseUrl`'s loopback-only allowlist against the README's "run it on your own infra" promise. Nothing here belongs in `../wip/inbox/` — every item is in-repo.
