...
- **`src/hooks/`** — `useRun<TInput,TOutput>`, the unified client state machine (`idle → running → done|error`) that dispatches blocking vs durable by `mode`. Holds the durable poll loop, the staleness token, the elapsed ticker, the wall-clock ceiling, the `classifyTransportError` wrapping, the **transient-failure budget** (a momentary 5xx/network blip on one poll tick — flagged `transient` by `pollDurableRun` — or a rejected poll await is retried up to `MAX_TRANSIENT_POLL_FAILURES`, surfacing `degraded` meanwhile, rather than abandoning a run that's still completing server-side), and the **bare-runner fallback** (a durable `start` that returns `lifecycle_unavailable` transparently re-runs the blocking path — a bare runner has no run store but also no ~30s gateway cap, so blocking is the right path there, mirroring the SDK's own `startAndWaitForResult`). Forms never branch on mode — they just call `run(input)`.
...
- **Durable** (`client.start` then poll) — survives the ~30s cap and streams coarse live status. Hosted-safe everywhere; the default (`NEXT_PUBLIC_EXECUTION_MODE`, defaults `"durable"`). On a bare runner with no run store it doesn't dead-end — `useRun` transparently falls back to the blocking path (see `src/hooks/`).
