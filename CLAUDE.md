...
- **Rule 1 — a layout names a path, and almost nothing more.** A design says "a `Textarea` here, bound to `/inputs/text`". It never restates a field's kind, its bounds or its choices; the descriptor still owns all of that, so a design cannot go stale about those and the everyday method change leaves the page rendering. The **almost** is a label and, on four of the five committed layouts, a `required` check on the gating input — the catalog offers `checks` to the model, so a produced layout carries its own copy and no gate compares it against the descriptor. What covers that is the third staleness question rather than the layout's silence: the record signs the hash of every source, so any `.mthds` edit reddens `design:check`. Narrowing the catalog is `mthds-form`'s to do; never edit a layout to close the gap.
