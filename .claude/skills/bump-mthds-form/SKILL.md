...
**If the installed version doesn't match the `package.json` range**, someone has probably hand-installed a local tarball (`npm pack` in `../mthds-form`, then `npm install <tgz>`) to develop the kernel against this starter. Unlike the SDK there is **no `make use-local`/`make use-npm` pair for this package**, so there is no one-shot restore target: tell the user, and offer `npm install @pipelex/mthds-form@latest` to get back to a clean published baseline before bumping. (`@latest` on purpose — the bare name would just re-resolve the stale caret range already in `package.json`.)
