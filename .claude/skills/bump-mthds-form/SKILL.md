...
- **Run `npm ls @json-render/core` too, and for the same reason one package over.** `package.json` names it as a **direct** dependency at the range the kernel itself declares — today `^0.20.0` on both sides — because `useRunInputs.ts` calls `createStateStore` and `DesignedPage.tsx` types a `StateStore`, and a host cannot import a transitive dependency. So the two ranges have to move together: npm reads a leading `0` as the major, so a kernel that moves to `^0.21.0` overlaps `^0.20.0` nowhere and two copies get installed. The store this repo creates is then a `StateStore` from one copy handed to a `GenerativePage` from the other, and `make typecheck` fails naming the same type twice. Read the kernel's own `dependencies` after the bump and match the range in `package.json` in the same commit. The ask that removes the direct dependency altogether — re-export the store factory and its types from `./generative` — is filed upstream as part of `L-260906-8e4500`.
