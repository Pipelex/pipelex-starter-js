...
The demo-free [`pipelex-method-apps`](https://github.com/Pipelex/pipelex-method-apps) template holds the reference copy of this scaffold and of the run chrome it writes against, and this gallery keeps a copy by hand: a fix lands in the template first. [`docs/chrome-lineage.md`](chrome-lineage.md) states the rule and lists the differences the gallery keeps on purpose — for the scaffold, the registry it writes into (`ExampleTabs.tsx`) and the name of the file cap (`MAX_PDF_BYTES`).
