...
- **No custom URL resolution.** `PdfForm` passes a `resolveUrl` that hands its argument straight back, which is not the affordance the name suggests — it is what keeps the kernel's Preview from spinning forever. The control previews a file it took through its own dropzone from an object URL it made; for any _other_ value it asks the host to resolve `value.url` and shows a spinner until that answers. The "Use sample PDF" shortcut writes a value the kernel never saw, so with no resolver at all its Preview never resolves. A `data:` URL is already displayable, so returning it unchanged is the whole job. Genuine resolution — turning a `pipelex-storage://` reference into something fetchable — is not built, and a form seeded from a previous run would need it.
