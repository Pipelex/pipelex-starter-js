...
The same function is keyed on the input **values**, not on the literal name `document`. A gate that reads `inputs.document` silently stops applying the day the bundle renames that input — codegen carries the rename into the form, the readiness rules and the wire envelope, and the byte cap just quietly disappears. Failing open on a routine edit is worse than not having the check.
