...
The `id` is a **dotted path**, not a name, which is what makes the same handler work for a file nested inside a structured concept or an item of a list. While the field's id sits in `env.uploadingIds`, the kernel shuts **every door into that value** — the dropzone, the "paste a URL instead" toggle and the URL input behind it — and shows a spinner. That guarantee is why this app needs no staleness token for a second selection while the first is still uploading, and why `PdfForm` passes plain run state as the form's `disabled`; the hook keeps the id there from the grant request until the reference is written, so there is no window between the two steps. The one write path `uploadingIds` cannot cover is the host's own chrome: the "Use sample PDF" shortcut writes into the same field — it fetches the sample and hands it to the same `dropFile` — so it disables itself while the field is resolving (`running || uploadingIds.size > 0`), the same rule, applied by the one who owns the button.

The kernel previews a file dropped into it from its own local copy, which it keeps after the upload. For a stored reference it holds no copy of — the sample's, or one pasted through "paste a URL instead" — it asks the form's `env.resolveUrl`, and `PdfForm`'s answer hands the reference straight back. The kernel judges a resolver's answer by its own URL gate, which refuses the storage scheme, so the preview settles on the kernel's placeholder. The method-app template resolves such a reference through a same-origin assets route, which this gallery has not taken.

### Why the file goes straight to storage

The first version of this seam encoded each file as a base64 `data:` URL in the browser and kept it in the form's value until Run, and the run's Server Action received every file of the form in one request, which `next.config.js` let reach 12 MB. The cap bounded one file, but nothing bounded a list, and no per-submission cap would have been right: a claim with fifteen receipts is a legitimate submission. An app made from the method-app template over a list of phone photos went past that limit, and the page could only say "Could not reach the server".
...
- **The type and the size are the grant action's.** `checkUploadRequest` (`src/lib/fileInputs.ts`) reads the request as untrusted JSON and refuses a type outside the method's `ALLOWED_MIMES` and a size past `MAX_FILE_BYTES`, before any grant is asked for. `ALLOWED_MIMES` lives in a module of its own (`src/types/summarizePdfUploads.ts` for the PDF example), because the action's `"use server"` file may export only async functions and the form reads the list too. The grant then signs both, so storage refuses a file that differs from what was declared. `MAX_FILE_BYTES` is the platform's own limit, and the grant route's `413` for a larger declaration stays the authority behind it.
- **The form offers only what the grant action grants.** `PdfForm` passes `ALLOWED_MIMES` to `useRunInputs` as `allowedMimes`, and the hook narrows each file input to those formats with the kernel's `narrowFileFormats`. The dropzone's hint, its file picker's filter and its own check then name the same list, so a PNG dropped on the PDF input is refused at the dropzone rather than after the grant action has been asked. The form can offer only a format the kernel knows for the input's kind, and a list naming none of an input's formats is refused when the form renders.
...
**The result view applies one rule of its own, and it is about URLs, not shapes.** It was written against kernel 0.8.0, which decided what to paint, link and frame from an `isViewableUrl` accepting `http:`, **any** `data:` media type, `blob:` and same-origin paths; behind that verdict sat an `<img>`, an `<a target="_blank">` and — whenever the payload's own `filename` or `mime_type` looked previewable — a `DocumentPreview` `<iframe>` carrying no `sandbox` attribute. So a value stating `url: "data:text/html,…"` and `filename: "report.pdf"` was framed, and the frame executed. `scrubResultUrls` (`src/lib/resultUrls.ts`) is the answer: it walks the result descriptor exactly as `checkFileInputs` walks the input one — the descriptor classifies, never the value's shape — and removes any file URL the kernel would act on that is not `https:` or a PNG/JPEG/WebP `data:` URL. Three properties are deliberate. It **reports** what it removed, and `<RunResult>` renders that as a note, because the JSON view is billed as the verbatim receipt and a payload it no longer shows in full has to account for the difference. It **normalizes** an accepted URL, so the string the policy judged is the string the kernel gets: `new URL(" https://…")` strips the leading space and the kernel's `/^https?:/` does not, which is how a validated `public_url` gets skipped in favour of an unvalidated `url`. And it **leaves alone** anything the kernel would never touch, `pipelex-storage://` included — stripping an inert reference buys no safety and costs the receipt.

This is a stopgap and its docstring says so, and since kernel 0.9.0 its reason is gone. The kernel now judges every URL it acts on through one exported gate, `viewableUrl`, which parses rather than prefix-matches, allows a `data:` URL only for the raster image types and PDF, and admits a document to the frame only over `http(s):` whatever its declared type. It also covers the half this module never could: an image in a text result's Markdown renders as a link, and loads only for a host that passes `proseImages="load"`. The module is due to be deleted, or reduced to a narrower host policy that calls `viewableUrl` rather than restating it.
...
- **No custom URL resolution.** The kernel previews `http(s):`, `data:` and `blob:` URLs directly, and a dropped file from its own local copy, and asks the host's `resolveUrl` only for a value it cannot paint, which here is a `pipelex-storage://…/x.pdf` it holds no copy of: the sample PDF's, which is stored the way a drop is, or one pasted through the control's own "paste a URL instead". `PdfForm` passes a resolver that hands the URI straight back. The kernel judges that answer by its own URL gate, refuses the storage scheme and settles on its placeholder, which beats the spinner it shows forever over a stored reference when no resolver is passed at all (L-260924-3827af). Genuine resolution — exchanging that reference for a signed web URL — is not built, and a form seeded from a previous run would want it. Pinned by a regression test.
