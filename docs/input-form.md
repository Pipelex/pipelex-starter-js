...
The `id` is a **dotted path**, not a name, which is what makes the same handler work for a file nested inside a structured concept. While the field's id sits in `env.uploadingIds`, the kernel disables the **dropzone** and shows a spinner, which is why this app needs no staleness token for a second drop. One gap to know about: `uploadingIds` reaches the dropzone only, not the control's "paste a URL instead" toggle and text input, so a URL pasted while an encode is in flight is overwritten when that earlier read lands. The host cannot close that door from here — it is filed upstream.
