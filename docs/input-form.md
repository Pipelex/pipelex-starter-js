...
The kernel previews a file dropped into it from its own local copy, which it keeps after the upload. For a stored reference it holds no copy of — the sample's, or one pasted through "paste a URL instead" — it asks the form's `env.resolveUrl`, and `PdfForm`'s answer hands the reference straight back, so the preview shows "Preview unavailable". The method-app template resolves such a reference through a same-origin assets route, which this gallery has not taken.
