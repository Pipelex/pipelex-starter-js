...
> - **#3** — Decision (revised after review): **kept `durable` as the default** (changing it to blocking would re-break the hosted majority — the whole point of the feature) and surface an **explicit error**, not a silent fallback. A self-hosted runner that lacks the polling endpoints is a real backend/config mismatch the developer must see — silently downgrading durable → synchronous `execute` would hide it. `useRun` therefore fails the run with the existing `lifecycle_unavailable` error, which **names the endpoint URL** and points at Blocking mode. (URLs change constantly in dev, so the URL is the key signal.) Separately closed a gap: the 5xx/4xx (`classifyResponse`) path now includes `API URL:` in Technical details too — `api_unreachable` and `lifecycle_unavailable` already named the URL, but plain response errors didn't.
>
...
> **✅ Resolved (release/v0.2.0, @pipelex/sdk 0.3.0).** Resolved upstream in the SDK rather than in the adapter: `@pipelex/sdk` 0.3.0's blocking `execute()` returns a `PipelexExecuteResult` whose `.main_stuff` getter resolves the output out of the working memory via the response's `main_stuff_name` — so the blocking path now delivers the **same resolved `main_stuff`** the durable path does. `executeBlockingRun` adapts onto `RunResults` with `{ pipeline_run_id, main_stuff: response.main_stuff }` (no `pipe_output`), and `findOutputContent` collapsed to a single `main_stuff` arm — the shape-guessing `pipe_output` search is gone entirely. The two modes agree by construction; a pipeline with an intermediate stuff matching the same predicate can no longer render the wrong result in blocking mode.
