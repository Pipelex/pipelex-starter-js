...
- **`src/lib/runInputs.ts` — the server-side input gate, sharing the kernel's rules with the browser.** `gateRunInputs` combines the per-input schemas, repairs the data, validates it, re-applies the kernel's emptiness predicates (`inputMustBeFilled` + `isFilled` — ajv's `required` only asserts key presence, so an empty string would otherwise pass), and builds the `{concept, content}` payload. The browser runs the kernel's `computeReadiness` for the Run button; the Server Action is the trust boundary, because a Server Action is a public endpoint, and stays a strict superset of what the button enforces. `requireContract` throws when a contract lookup misses, since an undefined contract otherwise renders as an empty form with a live Run button.
