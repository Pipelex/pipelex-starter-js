...
- **The sample PDF can be previewed.** The kernel previews a file it took through its own dropzone from an object URL it made; for any other value it asks the host to resolve `value.url` and spins until that answers. "Use sample PDF" writes a value the kernel never saw, so with no resolver its Preview spun forever. `PdfForm` now supplies one that returns a `data:` URL unchanged. A file the user replaces still previews as the earlier one — that state is private to the kernel's control, and the fix is upstream.
