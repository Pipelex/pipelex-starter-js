...
- **A URL pasted while the PDF form was still resolving a file could start a run that was then silently abandoned.** The kernel threads `uploadingIds` to the dropzone alone; its "paste a URL instead" toggle and input read the ambient `disabled`, which the form was passing as the bare run state. So during the sample PDF's fetch a pasted URL made the form ready, and the durable run it started was dropped client-side when the fetch landed and reset the run state — while that run went on executing and billing. The form now computes one `busy` flag covering both a run in flight and an input still resolving, passes it as the form's `disabled`, and gates submit on it. Reported upstream as well, since form-wide disabling is coarser than the per-field flag ought to need.
