...
- **`src/lib/runInputs.ts` — one input gate, two call sites.** `gateRunInputs` runs the kernel's four-step gate (combine the per-input schemas, repair the data, validate, build the `{concept, content}` payload). The browser runs it for the Run button's readiness; the Server Action runs it again as the trust boundary, because a Server Action is a public endpoint. `requireContract` throws when a contract lookup misses, since an undefined contract otherwise renders as an empty form with a live Run button.
