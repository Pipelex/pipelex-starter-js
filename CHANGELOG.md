...
- **`@pipelex/sdk` 0.24.0**: bumped from 0.20.1, which brings the upload grant (`requestUploadGrant`), the browser-safe `@pipelex/sdk/upload` entry, and an `uploadWithGrant` that bounds its own upload by the file's size, so `useFileInputs` passes it no signal and `uploadTimeoutMs` is removed. A failed upload is now named by the SDK's `UploadTransportError.code`: a server error from storage, or storage giving up on a stalled file, no longer reads as "Could not reach Pipelex storage". The other breaking changes in between are confined to the artifact-download helpers, which this app does not use.
